...
func GetStackQuestionsByClassification(classification string) ([]*StackQuestion, error) {
	...
	rows, err := db.Query("SELECT Title, Link, QuestionId, Tags, CreationDate FROM StackQuestions WHERE Classification = $1 and Readed = 0 ORDER BY CreationDate DESC LIMIT 20", classification)
